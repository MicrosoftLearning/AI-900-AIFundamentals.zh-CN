...
1. 选择“组件”。 搜索“汽车价格数据(原始数据)”数据集并将其放置在画布上。

1. 右键单击（在 Mac 上，按住 Ctrl 并单击）画布上的“汽车价格数据(原始数据)”数据集，然后选择“预览数据”。
...
1. 关闭“数据输出”窗口，以便可以在画布上看到该数据集，如下所示：
...
1. 双击“清理缺失的数据”模块，然后在右侧窗格中，选择“编辑列”。 然后在“要清理的列”窗口中，选择“使用规则”，在“包含”列表中选择“列名称”，在列名框中输入“bore”、“stroke”和“horsepower”，如下所示：
...
1. 在“基本信息”页上，选择“新建”，并将试验名称设置为“mslearn-auto-training”，然后选择“下一步”   。
...
1. 请等待几分钟，以便运行完成。 可以通过选择“资产”下的“作业”来查看作业的状态 。 在此处，选择“汽车价格训练”作业。 在此处可以看到作业完成的时间。 作业完成后，数据集现在已准备好用于模型训练。

1. 转到左侧菜单。 在“创作”下，选择“Designer”********。 然后从管道列表中选择“汽车价格训练”管道******。

## 创建训练管道

使用数据转换准备数据之后，可以使用这些数据训练机器学习模型。 请完成以下步骤来扩展“汽车价格训练”管道。

1. 确保左侧菜单已选中“Designer”，并且你已返回到“汽车价格训练”管道********。
...
1. 选择“配置和提交”，并使用名为“mslearn-auto-training”的现有试验来运行管道。

1. 试验运行需要 5 分钟或更长时间才能完成。 返回到“作业”页，选择最新的“汽车价格训练”作业运行 。

1. 试验运行完成后，右键单击“评分模型”模块，选择“预览数据”，然后选择“评分数据集”以查看结果  。
...
1. 完成试验运行需要几分钟。 返回到“作业”页，选择最新的“汽车价格训练”作业运行 。
...
1. 在“创建推理管道”下拉列表中，选择“实时推理管道” 。 几秒钟后，将打开名为“自动价格训练实时推理”的新版本管道。

1. 将新管道重命名为“预测自动价格”，然后查看新管道。 它包含用于提交新数据的 Web 服务输入，以及用于返回结果的 Web 服务输出。 此管道中包含某些转换和训练步骤。 训练的模型将用于对新数据进行评分。

    在接下来的步骤中，将对推理管道进行以下更改：

    ![指示了更改的推理管道的屏幕截图。](media/create-regression-model/inference-changes.png)

   在后续步骤中修改管道时，可参考此图。

1. 推理管道假设新数据将与原始训练数据的架构匹配，因此包含训练管道中的“Automobile price data (Raw)”数据集。 然而，此输入数据包括了模型预测的“price”标签，这对于尚未进行价格预测的新车数据来说并不具有说服力。 删除此模块，然后在“数据输入和输出”部分中，将其替换为“手动输入数据”模块 。
1. 编辑“手动输入数据”模块并输入以下 CSV 数据，其中包括三辆车的不带标签的特征值（复制并粘贴整个文本块）：
...
1. “评分模型”模块的输出包括所有输入特征以及预测的标签。 修改输出，使其只包含预测：
    ...
    ```
>**注意**：复制和粘贴可能会在 Python 脚本中引入不应有的空格。 反复检查 import、def 或 return 前是否不含空格******。 确保在 scored_results 和 scored_results.rename() 前缩进一个制表符****。

1. 将“评分模型”模块的输出连接到“执行 Python 脚本”的“Dataset1”（最左侧）输入  。

1. 将“执行 Python 脚本”模块的“结果数据集”（左侧）输出连接到“Web 服务输出”模块  。
...
1. 返回到“作业”页，选择最新的“汽车价格训练”作业运行（与“mslearn-auto-inference”试验相关的作业运行） 。

1. 管道完成后，右键点击“执行 Python 脚本”模块。 选择“预览数据”，然后选择“结果数据集”，查看输入数据中三辆车的预测价格。

1. 关闭“Result_Dataset”选项卡。
...
1. 在“预测汽车价格”作业运行页中，从顶部菜单栏中选择“部署” 。

    ![屏幕截图显示“预测汽车价格”推理管道的“部署”按钮。](media/create-regression-model/deploy-screenshot.png)

1. 在配置屏幕中，选择“部署新的实时终结点”并使用以下设置：
    - 名称：predict-auto-price
    - 描述：自动执行价格回归
    - **计算类型**：Azure 容器实例

1. 选择“部署”，然后等待几分钟，以便部署 Web 服务。
...
1. 当“predict-auto-price”终结点打开时，选择“测试”选项卡。我们将使用它来测试模型与新数据。 在“输入数据以测试终结点”下，删除目前的数据。 复制以下数据并粘贴到数据部分：  
