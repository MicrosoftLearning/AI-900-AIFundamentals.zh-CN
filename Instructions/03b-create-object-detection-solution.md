...
Microsoft Azure 中的“自定义视觉”服务提供基于云的解决方案，用于创建和发布自定义对象检测模型。 在 Azure 中，可以使用自定义视觉服务来基于现有图像训练物体检测模型。 创建物体检测解决方案有两个元素。 首先，必须使用标记图像训练模型以检测物体的位置和种类。 然后，在训练模型后，必须将其发布为可由应用程序使用的服务。
