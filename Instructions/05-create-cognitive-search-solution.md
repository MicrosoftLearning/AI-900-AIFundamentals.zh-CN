...
- **Azure AI 服务**资源，为搜索解决方案用于通过 AI 生成的见解来丰富数据源中的数据的技能提供 AI 服务。

    > 注意：你的 Azure 认知搜索和 Azure AI 服务资源必须位于同一位置！
...
你需要预配一个与 Azure 认知搜索资源位于同一位置的 Azure AI 服务资源。 搜索解决方案将使用该资源通过 AI 生成的见解来丰富数据存储中的数据。

1. 返回到 Azure 门户的主页。 单击“&#65291;创建资源”按钮，然后搜索“Azure AI 服务”。 选择创建一个 Azure AI 服务计划。 随后你会转到一个页面，可在其中创建 Azure AI 服务资源。 使用以下设置对其进行配置：
    - **订阅**：Azure 订阅。
